...
- 挖勺是按体积笼的六面体单元格整块移除，不是贴合勺面的几何切割：松开时中心落在勺碗椭球（外扩 0.06）内的单元格被删掉，然后自动补吃只沿边或顶点相连的单元和失去面接触的碎屑，保证剩余部分是一个闭合流形体。移除后重算边、质量和边界四边形，重建细分表面，节点位置和速度按网格编号带过去；新暴露的内部面按 `cut` 遮罩渲染成无焦糖的切面。
