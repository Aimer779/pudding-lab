...
| `src/scene/world.ts` | WebGPU、材质、照明、阴影、相机、勺子与勺上块动画、表面重建与资源释放 |
...
勺子只对碗心以下的节点做 25% 的逐子步推挤，压到底时不会与体积约束硬冲突；测试中把勺子压入顶面 0.3 个单位没有出现四面体翻转。松开时以勺碗椭球外扩 0.08 判定单元中心，默认一口大约 3–12 个单元；逐口吃完整个布丁约 70 口，每口后 0.5 秒内保持有限且无翻转。表面重建（含两层细分 stencil）在本机 Node 中约 5–8 ms，作为一次性事件触发。
