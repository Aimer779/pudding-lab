...
import { attribute, mix, mx_noise_float, positionLocal, uniform } from 'three/tsl';
...
export async function createWorld(canvas:HTMLCanvasElement,sim:Simulation) {
  ...
  material.colorNode=mix(mix(bodyColor,topColor,g),mix(cutColor,bodyColor,torn.mul(.45)),c);
  material.roughnessNode=mix(mix(.34,.18,g),torn.mul(.28).add(.5),c);material.clearcoatNode=mix(mix(.22,.85,g),.03,c);
