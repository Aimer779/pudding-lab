...
export function mountPanel() {
  element('#app').innerHTML=`
    ...
    <main id="stage" aria-label="Interactive pudding"><canvas id="scene" aria-label="Drag the pudding to stretch it, or switch to the spoon and press to scoop a bite. Use the nudge and Take a bite buttons for keyboard alternatives."></canvas><div id="scene-message" role="status">Preparing something sweet<span></span></div><button id="retry" hidden>Try again</button></main>
    <section class="caption"><p>Grab. Stretch. Let go.</p><span>Or pick up the spoon and dig in.</span><div class="readings"><div><output id="volume">100.0%</output><small>REST VOLUME</small></div><div><output id="motion">0.00</output><small>MOTION</small></div><div><output id="eaten">0%</output><small>EATEN</small></div><div><output id="state">Settling</output><small>THE MOMENT</small></div></div></section>
    <aside class="controls" aria-label="Pudding controls">
      ...
      <div class="tools" role="group" aria-label="Tool"><button id="tool-grab" data-tool="grab" aria-pressed="true">Fingers</button><button id="tool-spoon" data-tool="spoon" aria-pressed="false">Spoon</button><button id="bite">Take a bite</button></div>
      ...
      <details id="about"><summary>Inside the experiment <span>+</span></summary><p>A soft solid, with a sweet side. Grab a little piece and feel the rest follow.</p><p>Shape is simulated with volume constraints. Motion is measured from the moving pudding. Flavor changes its appearance.</p><p>The spoon removes real cells of the volume mesh. What is eaten is gone until you reset.</p><small>THREE.JS · WEBGPU · XPBD</small><a href="?inspect=1" class="inspect-link">Open rendering diagnostics ↗</a></details>
    </aside>
